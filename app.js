...
const gameboard = (function () {
  ...
  const _removePlayListeners = (index = undefined) => {
    if (index === undefined) {
      ...
      _singleSquares[index].removeEventListener("click", _handlePlayerMove);
      _singleSquares[index].removeEventListener("mouseover", _handleMouseOver);
      _singleSquares[index].removeEventListener("mouseout", _handleMouseOut);
    }
  };
  ...
  const _checkHorizontals = (bestMoveEvaluation = false) => {
    for (let i = 0; i < 8; i = i + 3) {
      if (
        ...
        if (_gameboardArr[i] === gameController.getCurrentPlayer().symbol) {
          return 10;
        } else {
          return -10;
        }
  ...
  const _checkForWinners = () => {
    let gameWon = _checkHorizontals() || _checkVericals() || _checkDiagonals();
    return gameWon;
  };
  ...
  const evaluateBoard = () => {
    let gameScore = 0;
    gameScore +=
      _checkHorizontals(true) + _checkVericals(true) + _checkDiagonals(true);
    return gameScore;
  };
